...
const determineHandType = (hand: string) => {
	...
	let extractedJoker = 0;
	let extractedJokerFromCards = Object.fromEntries(
		Object.entries(groupedCards).filter(([key, value]) => {
			if (key === 'J') {
				extractedJoker += value;
			}
			return key !== 'J';
		})
	);
	...
	if (extractedJoker === 5) {
		extractedJokerFromCards = { A: 0 };
	}

	const sortedCards = Object.values(extractedJokerFromCards).sort((a, b) => b - a);

	sortedCards[0] += extractedJoker;
