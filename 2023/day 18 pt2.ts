const mappedData: [string, number, string][] = [...realData]
	...
	.map((d) => {
		const color = d[2];
		const readableColor = color.replace(/\(|\)/g, '');
		const realSteps = Number(readableColor.slice(0, -1).replace('#', '0x'));
		...
		const realDir = directions[readableColor.slice(-1)];

		return [realDir, realSteps, color];
	});
