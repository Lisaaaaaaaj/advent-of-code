...
function generate(unicodeText: string): string {
	let x: number[] = [];
	let k;
	let AA;
	let BB;
...
for (let count = 0; count < Number.MAX_SAFE_INTEGER; count++) {
	const str = generate(`${realData}${count}`).slice(0, 5);
	if (str.split('').every((s) => parseInt(s, 10) === 0)) {
		ans += count;
		break;
	}
}
