	...
	const floodFill = (grid: number[][], startX: number, startY: number, endX: number, endY: number, instruction: string) => {
		...
		while (queue.length > 0) {
			...
			grid[x][y] += instruction === 'toggle' ? 2 : instruction === 'turn on' ? 1 : grid[x][y] > 0 ? -1 : 0

			for (const [dx, dy] of directions) {
				const newX = x + dx;
				const newY = y + dy;

				if (!seen.has(JSON.stringify([newX, newY])) && newX >= startX && newX <= endX
	...
	workableData.forEach(d => {
		const [x1, y1, x2, y2] = d.trim().match(/[0-9]+/g)!.map(dd => parseInt(dd, 10))
