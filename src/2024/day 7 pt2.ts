...
const findAllEquationCombinations = () => {
	workableData.map((d) => {
		const characters = ['+', '*', 'c'];
		const relArr = characters;
		const minCombi = d.combinationsLength;
		const maxCombi = d.combinationsLength;
		const combinations = [
			...new Set(
				[...Array(maxCombi).keys()]
					.reduce(
						(result) => [
							...new Set([...relArr, ...result.flatMap((val) => relArr.map((char) => val + char))]),
						],
						[] as string[]
					)
...
const chooseOperator: { [key: string]: (arg0: number, arg1: number) => number } = {
	...
	'*': (a: number, b: number) => (a *= b),
	c: (a: number, b: number) => parseInt(`${a}${b}`, 10),
};

console.log(allEquationCombinations);
...
const totalCalibrationResults = allEquationCombinations
	.map((d, inddex) => {
		...
		console.log(inddex, [...new Set(nums.filter((n) => n === total))]);
