...
let foundTotalRemovedAccessableRollsOfPaper = 0;
let search = true;

while(search) {
	let foundAccessableRollsOfPaper = 0;
	let foundAccessableRollsOfPaperAt: [number,number][] = [];
		...
		for (let x = 0; x < numRows; x++) {
			for (let y = 0; y < numCols; y++) {
				...
				if (foundRollsOfPaper < 4) {
					...
					foundAccessableRollsOfPaperAt.push([y,x])
				}
			}
		}
	};
	...
	if (foundAccessableRollsOfPaper === 0) {
		search = false;
		break;
	} else {
		foundTotalRemovedAccessableRollsOfPaper += foundAccessableRollsOfPaper;
	}
	const removeAccessableFoundRollsOfPapers = (grid:string[][]) => {
		foundAccessableRollsOfPaperAt.forEach(roll => {
			grid[roll[0]][roll[1]] = '.';
		})
	}
	removeAccessableFoundRollsOfPapers(workableData);
}

console.log(foundTotalRemovedAccessableRollsOfPaper)
