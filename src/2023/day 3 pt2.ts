...
realData.map((line) => {
	...
	[...line].map((linePart, linePartIndex) => {
		...
		if (currentLinePartIsSymbol || currentLinePartIsNumber) {
			...
			if (currentLinePartIsSymbol || (currentLinePartIsNumber && !nextLineIsNumber)) {
	...
	if (currentLineIndex > 0) {
		currentLine.map((line) => {
			if (line.num === '*') {
				...
				const relevantNumbsToMultiply: Array<number> = [];
				...
				const pushAndFilterRelevantNum = (
					...
					return revLine.map((rvLine) => {
						if (
							...
							relevantNumbsToMultiply.push(Number(rvLine.num));
						}
				...
				if (relevantNumbsToMultiply.length === 2) {
					console.log(relevantNumbsToMultiply);
					num += relevantNumbsToMultiply[0] * relevantNumbsToMultiply[1];
				}
			}
		});
	}
